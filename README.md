...
- **M5StickV設定**: 事後閲覧方式／プレビュー確認方式の切り替え（詳細は後述
  「Web上での確認・印刷モード」参照。自動俳句印刷モードは下の「俳句設定」カードに移動した）
  に加えて、「回転」ボタンで写真の初期回転（0/90/180/270°、
...
- **俳句設定**（M5StickV設定のすぐ下）: 生成する詩の形式（俳句＝五七五・3行、または
  ポエム＝30文字程度の自由詩）と、印刷時に添える著者名（任意）を設定する。どちらも
  ATOM Lite本体のNVSに保存され再起動後も保持される（`src/haiku.*`、`/api/haiku/settings`）。
  旧・M5StickV設定カードにあった「自動俳句印刷モード」（M5StickV撮影のたびに自動生成・
  印刷する設定）もここに移動した。詩の形式はATOM Lite本体のボタンをダブルクリックしても
  切り替えられ、本体LEDの色（緑=俳句／青=ポエム）で現在のモードがわかる（詳細は後述
...
が現在のモードを示す（詳細は前述「ATOM Lite本体のボタン・RGB LED」）。M5StickVカメラ
経由の写真については、同じ「俳句設定」カードで「自動俳句印刷モード」を選んでおくと、この節で
説明する処理（生成→印刷）を撮影のたびに自動実行できる（詳細は前述
...
| GET | `/api/haiku/settings` | 俳句設定のJSON (`poemType`=`haiku`/`poem`, `author`) |
| POST | `/api/haiku/settings` | `poemType`,`author` (form) で俳句設定を更新（再起動後も保持） |
