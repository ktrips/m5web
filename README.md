...
- **QRコードを追加**: URLを入力して「保存」すると、以後アップロード画像・俳句プリントの右下に
  小さなQRコードを自動で重ねる（M5StickV直接印刷には非適用、空欄なら何も付かない、短縮URL
  推奨。`/api/qrwatermark/settings`、ATOM Lite本体のNVSに保存され再起動後も保持）。同じ
  URL欄を使って「QRコードを印刷」ボタンを押すと、そのURLだけを単独のQRコードとして印刷
  できる（`/api/print/qr`。QR自体の生成はブラウザ側ではなくプリンター内蔵の2Dシンボル生成
  機能、GS ( k コマンドで行う）。かつて設定タブに別カードとしてあった「QRコード埋め込み」は
  このカードに統合され廃止した。
