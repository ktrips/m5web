...
`http://m5web.local`（またはIPアドレス）を開くと、ヘッダーと接続状況の下に「カメラ・画像」
「設定」の2タブがある（元は「カメラ」「アップロード」「設定」の3タブだったが、前者2つを
統合した）。

### カメラ・画像タブ

- **画像デフォルト: 明るさ・コントラスト・回転**: M5StickVカメラ・写真アップロードの両方に
  使われる明るさ・コントラストの初期値（-100〜100、既定0）と、M5StickVカメラの写真の
  デフォルト回転（0/90/180/270°）をまとめたカード。タブの一番上に配置。ATOM Lite本体のNVSに
  保存され再起動後も保持される。明るさ・コントラストは**次に受信するフレーム／次に選択する
  写真から**サーバー側で適用され（既に受信・表示中の1枚には遡って反映されない）、写真
  アップロードは「画像を印刷」カードで個別調整もできる（この個別調整はデフォルト値自体を
  書き換えない）。`/api/print/photo`・`/api/print/photo/url`（外部プログラムからの写真送信、
  後述）で送られたJPEGにも、この値がそのまま適用される（M5StickVカメラと同じ扱いで、個別
  調整の余地は無い）。回転は**次に受信するフレームから**ディザリング後の1bppビットマップに
  対して適用される（同じく遡って反映されない）——「カメラ（M5StickV/CamS3）」カードにある
  「回転」ボタン（後述）とは別物で、こちらは今後受信するすべてのフレームに効く既定値、
...
- **ギャラリー**: M5StickVカメラで撮った写真・アップロードした写真は、印刷時に自動的に
  ...
  不要な写真は「削除」ボタンで消すこと。
- **俳句設定**（画像を印刷のすぐ下）: 生成する詩の形式を「俳句（五七五・3行）」「ポエム
  （30文字程度の自由詩）」「作らない（デフォルト）」の3択で1つにまとめて選ぶ
  （`src/haiku.*`、`/api/haiku/settings`）。俳句・ポエムを選ぶとM5StickV撮影・写真
  アップロードのどちらでも撮影・選択のたびに自動生成して編集可能なボックスに表示する
  （設定タブ「自動化設定」カードとは独立した設定）。生成後に印刷するかどうかは設定タブ
  「自動化設定」カードの「俳句が生成されたら自動印刷」チェックボックスで決まる（デフォルト
  オフ＝「俳句プリント」ボタンを押すまで印刷されない）。著者名（任意）を入力すると印刷時に
  左下へ小さく添えられる。本体LEDの点滅色（緑=俳句／青=ポエム）も参照（詳細は後述
  「ATOM Lite本体のボタン・RGB LED」）。
- **QRコードを追加**（俳句設定のすぐ下）: URLを入力して「保存」すると、以後アップロード画像・
  俳句プリントの右下に小さなQRコードを自動で重ねる（M5StickV直接印刷には非適用、空欄なら
  何も付かない、短縮URL推奨。`/api/qrwatermark/settings`、ATOM Lite本体のNVSに保存され
  再起動後も保持）。同じURL欄を使って「QRコードを印刷」ボタンを押すと、そのURLだけを単独の
  QRコードとして印刷できる（`/api/print/qr`。QR自体の生成はブラウザ側ではなくプリンター
  内蔵の2Dシンボル生成機能、GS ( k コマンドで行う）。かつて設定タブに別カードとしてあった
  「QRコード埋め込み」はこのカードに統合され廃止した。

### 設定タブ

- **自動化設定**（タブの一番上）: 「画像が転送されてきたら自動印刷」「俳句が生成されたら
  自動印刷」の2つのチェックボックス。どちらもデフォルトはオフ。
  - **画像が転送されてきたら自動印刷**: M5StickVから写真が届いたときに、すぐ印刷する
    （オン）か、カメラ・画像タブの「カメラ（M5StickV/CamS3）」カードに表示して「印刷」
    または「破棄」を選ぶまで待機する（オフ、デフォルト）かを切り替える。内部的には
    従来の「M5StickV設定」カードと同じ`CameraLink::Mode`（`/api/camera/mode`）を
    操作しているだけで、見た目がラジオボタンからチェックボックスに変わり、カード自体が
    このカードに統合された（詳細は後述「Web上での確認・印刷モード」）。
  - **俳句が生成されたら自動印刷**: 俳句・ポエムの生成が完了した瞬間に自動的に印刷する
    （オン）か、「俳句プリント」ボタンを押すまで待つ（オフ、デフォルト）かを切り替える
    （`Haiku::autoPrint()`、`/api/haiku/settings`の`autoPrint`）。俳句設定カードの
    「俳句／ポエム／作らない」（自動生成するかどうか）とは独立していて、自動生成・手動
    「俳句を作る」ボタンのどちらで生成した場合にも同じように効く。
  
  明るさ・コントラスト・回転のデフォルト値はカメラ・画像タブの「画像デフォルト:
  明るさ・コントラスト・回転」カード（前述）に移動した。
...
m5webページの設定タブ「自動化設定」カード（タブの一番上）の「画像が転送されてきたら
自動印刷」チェックボックスで、受信した写真の扱いを2種類から選べる（設定はATOM Lite本体の
NVSに保存され再起動後も保持される。内部的には従来の「M5StickV設定」カードと同じ
`CameraLink::Mode`を操作しているだけで、カードの場所と見た目がラジオボタンから
チェックボックスに変わった）。

- **オン（事後閲覧方式）**: 受信次第すぐ印刷し、直近の1枚をカメラ・画像タブの
  「カメラ（M5StickV/CamS3）」カードで確認できる（印刷前の確認・キャンセルはできない）。
- **オフ（プレビュー確認方式、デフォルト）**: 受信してもすぐには印刷せず、
  「カメラ（M5StickV/CamS3）」カードに表示して「印刷」または「破棄」を選ぶまで待機する。

俳句・ポエムの自動生成は、この設定とは独立した「俳句設定」カード（俳句／ポエム／作らない、
詳細は[俳句生成](#俳句生成openai連携)）で選ぶ。俳句かポエムを選ぶと、m5webページを開いている
ブラウザが新しいフレーム（`frameSeq`の変化）や写真アップロードを検知するたびに、自動的に
［俳句を作る］を実行して編集可能なボックスに表示する。生成後に印刷するかどうかは、同じ
「自動化設定」カードの「俳句が生成されたら自動印刷」チェックボックス次第——**デフォルトは
オフ**で、「俳句プリント」ボタンを押すまで印刷されない。オンにすると、自動生成・手動
「俳句を作る」どちらで生成した場合でも、生成が終わった瞬間に自動で印刷される。写真の印刷は
ATOM Lite側だけで完結するが、俳句の生成はOpenAIへのアクセスも含めてブラウザ側の処理に
依存するため、**m5webページをブラウザで開いたままにしておく必要がある**（閉じている間に
受信した写真は、通常どおり写真だけ印刷され俳句は生成されない）。OpenAI APIキーが未設定の
場合や、複数枚が短時間に連続で届いた場合（前の俳句生成中に次のフレームが届いた場合など）は、
その回の俳句生成がスキップされることがある。
...
明るさ・コントラストの初期設定はカメラ・画像タブの「画像デフォルト: 明るさ・コントラスト・回転」
カードにまとめてあり（写真アップロードと共通、詳細は前述「カメラ・画像タブ」参照）、こちらも
**次に受信するフレームから**
...
m5webのギャラリー（前述「カメラ・画像タブ」の「ギャラリー」カード——M5StickVカメラで撮った写真・
...
- **本体上部の電源ボタンをダブルタップ（700ms以内に2回押し）**: m5webページの「俳句設定」
  カード（生成形式・著者名）をこの画面で確認・一部操作できる。もう一度ダブルタップすると
  ギャラリー表示に戻る。この画面が表示されている間、ボタンの役割がギャラリー表示中とは変わる:
  - **電源ボタンを1回だけ押す**（ダブルタップにならないよう700ms以上間隔を空ける）: 俳句⇄ポエムを
    切り替える。
  - **左ボタン（BtnA）／中央ボタン（BtnB）**: 自動生成（作らない／自動生成）を前へ／次へと
    切り替える。
...
## ATOM Lite⇔CamS3 直結シャッター

ATOM Matrix連携（前述）と同じ発想を、3台目のボードを使わず**ATOM Lite自身**からも使える。
CamS3のシャッターを起動する`src/cams3_remote.*`の`triggerShutter()`（ATOM Lite本体のボタン
シングルクリックで呼ばれる、[使い方](#使い方)参照）は、WiFi経由のHTTPリクエストに加えて
「ATOM LiteのGPIOから直接CamS3のGPIO0へパルスを送る」配線直結モードにも切り替えられる。
設定タブの「CamS3リモートシャッター」カードで選択する（デフォルトはWiFi経由、既存動作を
変えない）。
...
| ATOM Lite | CamS3 | 備考 |
|---|---|---|
| G25 | GPIO0 | ボタン押下時のみ約250ms（`kWiredPulseMs`）だけLOWを出力、他は常時HIGH |
| GND | GND | 信号線を意味あるものにするため必須 |

G25はATOM Lite側で他に使っていないピンから選定（G32/G26はM5StickVとのUART、G23/G33は
プリンターUART既定値、G27は内蔵LED、G39はボタン自体）。ATOM Matrix連携と同じく、
**CamS3のGPIO0はブートストラップピン**——ATOM Lite側がLOWを出している最中にCamS3の電源を
入れる／リセットすることは避けること。

### 使い方・制約

- 設定タブ「CamS3リモートシャッター」カードで「配線直結」を選ぶと、以後シングルクリックは
  WiFiを一切使わずGPIO25のパルス送信のみになる（`/api/cams3/settings`の`connMode`パラメータ、
  `wifi`|`wired`）。
- **ダブルクリックでの「CamS3の最新ギャラリーをもう一度印刷」指示は、配線直結モードでも常に
  WiFi経由**のまま——1本の配線では「シャッター」と「再印刷」という2種類の指示を区別して
  送れないため。CamS3が直接接続モード（`PrinterMode::kDirect`）でギャラリーを持たない場合と
  同様、失敗しても静かに無視される。
- 配線直結は戻り配線が無い一方向通信——`triggerShutter()`が`true`を返すのは「パルスを送った」
  ことの確認でしかなく、CamS3が実際に撮影・転送・印刷に成功したかどうかは分からない
  （ATOM Matrix連携と同じ制約）。実際の結果はCamS3自身のLED（GPIO14）やシリアルログで
  確認すること。
- WiFi/mDNSに依存しないため、ネットワークが不安定・CamS3の`m5cam.local`解決が失敗する環境
  でも確実に動く——[前回の調査](#atom-matrix連携camS3の有線シャッター)で触れたWiFi経由リモート
  シャッターの不調に対する、ATOM Matrixを使わない代替手段。
...
カメラ・画像タブの「カメラ（M5StickV/CamS3）」カード・「画像を印刷」カード、どちらにも「俳句を作る」
（ポエム選択時は「ポエムを作る」に表示が変わる）ボタンがあり、表示中の写真から季語入りの
五七五俳句、または30文字程度の自由なポエムをその場で生成できる。生成する形式は同じ
カメラ・画像タブの「俳句設定」カードで切り替える（`src/haiku.*`、ATOM Lite本体のNVSに保存）。
本体LEDの点滅色（緑=俳句／青=ポエム）が形式を示す（詳細は前述「ATOM Lite本体のボタン・RGB
LED」）。「俳句設定」カードで「俳句」または「ポエム」を選んでおくと、M5StickV撮影・写真
アップロードのどちらでも撮影・選択のたびに自動生成する（この節で説明する処理を自動実行
する）。生成後に印刷するかどうかは設定タブ「自動化設定」カードの「俳句が生成されたら
自動印刷」チェックボックス次第（デフォルトはオフ＝「俳句プリント」ボタンを押すまで
印刷されない）。デフォルトの「作らない」では、ボタンを押すまで俳句・ポエムは生成されない。
この自動生成の設定は、同じ「自動化設定」カードの「画像が転送されてきたら自動印刷」
（＝写真自体の自動印刷、詳細は前述「Web上での確認・印刷モード」）とは独立している。
以下は手動・自動どちらの場合にも共通の仕組み。
...
- カメラ・画像タブ「画像デフォルト: 明るさ・コントラスト・回転」の値が、M5StickVカメラと
  同じ扱いでサーバー側でそのまま適用される（`src/jpeg_print.cpp`が`CameraLink::status()`から
  読み取る）。写真アップロードのように個別調整はできないため、暗すぎる／薄すぎる場合は
  そちらの値を変更すること。
- 印刷したディザリング後の画像は、カメラ・画像タブの**「カメラ（M5StickV/CamS3）」カードにも表示される**
