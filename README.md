...
- **俳句設定**（M5StickV設定のすぐ下）: 生成する詩の形式（俳句＝五七五・3行、または
  ...
  詩の形式はATOM Lite本体のボタンをダブルクリックしても切り替えられ、本体LEDの色
  （緑=俳句／青=ポエム）で現在のモードがわかる（詳細は後述「ATOM Lite本体のボタン・RGB LED」）。
...
再印刷できる（5秒以上長押しするとWi-Fi設定リセットになるので、短押しであること）。**同じ
ボタンをダブルクリック**すると、俳句/ポエムの生成モードを切り替えられる（詳細は後述
「ATOM Lite本体のRGB LED」、および[俳句生成](#俳句生成openai連携)）。
...
合わせて、この1つのボタン・1つのLEDだけで「印刷」「モード切替」「Wi-Fiリセット」の3つの操作系を
まかなっている。

- **単押し（5秒未満、直後にもう一度押されない）**: そのときの直近フレームを再印刷する。
- **ダブルクリック（2回の単押しが400ms以内）**: 俳句設定カードの「俳句」「ポエム」生成モードを
  切り替える（`Haiku::poemType()`、`src/main.cpp`の`togglePoemMode()`）。単押しの判定は、2回目の
  押下が400ms以内に来なかった場合にのみ確定するため、単押しの反応は最大400ms遅れる。
- **長押し（5秒以上）**: Wi-Fi設定を消去してAPモードに戻る。

LEDの色そのものが**現在の生成モード**を表す——**緑＝俳句モード、青＝ポエムモード**
（`Led::setModeColor()`、起動時はNVSに保存された値を復元）。そのうえで、点灯パターンは
以下の通り（点滅・点灯とも、このモード色で光る）。

- **点滅5回**: 新しい写真がギャラリーに保存されたとき（M5StickVカメラ撮影・写真アップロード
  印刷のどちらでも、自動印刷/プレビュー確認方式を問わず発生）。
- **点灯（点滅なし・印刷できる写真があるあいだずっと点灯）**: 次のいずれかが真のあいだ点灯し続ける。
...
「俳句設定」カードで切り替える（`src/haiku.*`、ATOM Lite本体のNVSに保存）。同じカードで
ATOM Lite本体のボタンをダブルクリックしても切り替えられ、本体LEDの色（緑=俳句／青=ポエム）
が現在のモードを示す（詳細は前述「ATOM Lite本体のボタン・RGB LED」）。M5StickV撮影・写真
