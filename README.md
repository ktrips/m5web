...
- **俳句設定**（画像を印刷のすぐ下）: 生成する詩の形式を「俳句（五七五・3行）」「ポエム
  ...
  （M5StickV設定の事後閲覧／プレビュー確認モードとは独立した設定）。**印刷は常に手動**——
  自動生成された俳句・ポエムであっても、「俳句プリント」ボタンを押すまで印刷されない
  （かつての「自動でプリントまで実行」モードは、誤って意図しない印刷をしてしまいやすい
  ため廃止した）。著者名（任意）を入力すると印刷時に左下へ小さく添えられる。本体LEDの
  点滅色（緑=俳句／青=ポエム）も参照（詳細は後述「ATOM Lite本体のボタン・RGB LED」）。
...
- **M5StickV設定**: 事後閲覧方式／プレビュー確認方式の切り替え（詳細は後述
  ...
  俳句・ポエムの自動化とは独立している**）。明るさ・コントラスト・回転のデフォルト値は
  カメラ・画像タブの「画像デフォルト: 明るさ・コントラスト・回転」カード（前述）に移動した。
...
- **事後閲覧方式**: 受信次第すぐ印刷し、直近の1枚をカメラ・画像タブの
...
［俳句を作る］を実行して編集可能なボックスに表示する——**印刷は常に「俳句プリント」ボタンを
押した時だけ**（自動印刷は無い）。写真の印刷はATOM Lite側だけで完結するが、俳句の生成は
OpenAIへのアクセスも含めてブラウザ側の処理に依存するため、**m5webページをブラウザで開いた
ままにしておく必要がある**（閉じている間に受信した写真は、通常どおり写真だけ印刷され俳句は
生成されない）。OpenAI APIキーが未設定の場合や、複数枚が短時間に連続で届いた場合（前の俳句
生成中に次のフレームが届いた場合など）は、その回の俳句生成がスキップされることがある。
...
する）——ただし**印刷は常に「俳句プリント」ボタンを押した時だけ**で、自動印刷は無い。
デフォルトの「作らない」では、ボタンを押すまで俳句・ポエムは生成されない。この自動化は
