...
（Wi-Fi接続状況・IPアドレス・現在時刻・閲覧端末の位置情報——いずれもブラウザ側で完結する
表示で、時刻・位置情報はCamS3本体ではなくアクセスした端末のもの）があり、各カードの見出しには
絵文字アイコン（📸撮影・🖼️ギャラリー・🔀接続方式など）を付けて一目で内容がわかるようにして
ある。プリンターの接続方式を2つのモードから選べる（設定タブ「接続方式」カード、
`arduino/camS3/printer_mode.*`。切り替えは即再起動）:
...
**撮影方式**（設定タブ、`arduino/camS3/own_camera.*`）は接続方式とは独立していて、
**ATOM経由・直接接続どちらでも同じように効く**: 事後閲覧方式（自動印刷/自動転送）／
プレビュー確認方式（**デフォルト**——撮影しても印刷・転送はせず、「撮影」カードに表示された
写真を確認してから「印刷」を押すまで待つ。ATOM経由モードでは「印刷」を押した時点で初めて
ATOMへ転送される）を選べる。m5webのM5StickVカメラ設定と同じ考え方だが、こちらはCamS3自身の
カメラ用の設定で独立している。
...
- **CamS3自身のページ（`http://m5cam.local`）の「撮影」ボタン**（標準の方法。CamS3には
  ...
  よる競合リスクを避けている）。CamS3本体にはスピーカーが無いため、「撮影」ボタンを押した
