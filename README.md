...
合わせて、この1つのボタン・1つのLEDだけで「印刷」「Wi-Fiリセット」の2つの操作系をまかなっている。

- **ダブルクリック（2回の単押しが400ms以内）**: そのときの直近フレームを再印刷する。単押し
  1回だけでは何も起きない——本体のボタンは小さく露出しているため、誤って触れただけで
  印刷されてしまわないよう、意図的にダブルクリック必須にしている。
...
- **点滅3回（固定で緑——生成モードによらず常に緑）**: 上記のボタンのダブルクリックで印刷が
  成功したとき（`Led::notifyPrintDone()`）。生成モード色ではなく固定の緑にしているのは、
  ポエムモード（青）中でも「印刷できた」がひと目でわかるようにするため。
