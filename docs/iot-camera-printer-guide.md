...
さらに、**本体上部の電源ボタンを700ミリ秒以内に2回押す（ダブルタップ）**と、m5webページの「俳句設定」カード（生成する詩の形式・著者名・印刷の自動化）をこの画面で確認できます。もう一度ダブルタップするとギャラリー表示に戻ります。

この画面が表示されている間は、ボタンの役割がギャラリー表示中とは変わります。

- **電源ボタンを1回だけ押す**（ダブルタップにならないよう700ミリ秒以上間隔を空けて）：俳句⇄ポエムを切り替えます。
- **BtnA／BtnB**：印刷の自動化（俳句を作らない／自動で俳句作成／自動でプリントまで実行）を前へ／次へと切り替えます（BtnCはこの画面では何もしません）。
