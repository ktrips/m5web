...
**本書での使い方との対応**：`m5paper.ino`はボタンA/Bをギャラリーの前へ/次への送りに、ボタンCの短押しを「表示中の写真を再印刷」、長押し（約800ミリ秒）を俳句設定画面への切り替えに割り当てています（7.3節）。RGB LED2個は電源投入中の常時緑点灯と、ビープ音に合わせた点滅に使っています。電源ボタンは単押しで電源ON、長押しでOFFという機器本来の挙動のままで、本書のスケッチからは操作しません（実機で押すと電源管理IC「M5PM1」のブラウンアウト検出が働きESP32がリセットされてしまうことが確認できたため——詳細は7.4節）。SHT40・RX8130CE・オーディオコーデック・IR送信・microSDスロットは、いずれも本書のスケッチでは未使用です（ビープ音の再生にはES8311コーデック経由のスピーカーを使っています）。E Ink Spectra 6ディスプレイは6色対応ですが、印刷画像自体がすでに1bpp白黒ビットマップのため、本書ではWHITE/BLACKの2色しか使っていません（7.4節）。
...
| BtnC（右） | 短押し＝印刷（表示中の写真を再印刷）／長押し（約800ms）＝俳句設定画面に切替 |

どのボタンを押しても短いビープ音で反応を確認でき、BtnA/BtnBを素早く2回押すと区別できる二重ビープが鳴る（詳細は第7章）。ビープ音が鳴っている間はRGB LEDも一瞬消灯して点滅するため、音と光の両方で操作が反映されたことを確認できます。M5PaperColorはES8311コーデック＋1Wスピーカーを内蔵しているため、この音声フィードバックには追加の部品は不要です。
...
`http://m5web.local`を開くと、画面上部にヘッダーと接続状況、続けてギャラリーカードが表示され、その下に「カメラ」「アップロード」「設定」の3つのタブがあります（既定で開いているのは「カメラ」タブです）。

**ギャラリーカード（タブの外・常に表示）**

- 印刷した写真（M5StickV経由・アップロード経由の両方）は、自動的にATOM Lite本体のフラッシュメモリ（LittleFS）に保存され、一覧表示されます。各写真にはサムネイル、検出ラベル（あれば）、保存日付が表示され、「再印刷」「削除」ができます。**最大20枚まで**保存され、いっぱいになると新しい保存はスキップされます（この場合シリアルログに記録が残ります）。定期的に不要な写真を削除する運用が必要です。
...
- **デフォルト：明るさ・コントラストカード**：M5StickVカメラ経由の写真とアップロード写真の両方に使われる初期値です（-100〜100、既定は0）。ATOM Lite本体のNVS（不揮発性メモリ）に保存されるため、再起動しても値は保持されます。ただし反映のタイミングに注意が必要です。M5StickVカメラ経由の写真は**次に受信するフレームから**この値が適用されます（すでに受信・表示中の1枚には遡って反映されません）。アップロード写真は**次に選択する写真の初期値**として使われ、選択後はプレビュー画面で個別調整できます（この個別調整はデフォルト値自体を書き換えません）。`/api/print/photo`・`/api/print/photo/url`（8.5節、外部プログラムからの写真送信）で送られたJPEGにも、M5StickVカメラと同じ扱いでこの値がサーバー側でそのまま適用されます（個別調整の余地は無いため、暗すぎる・薄すぎる場合はこのカードの値自体を変更してください）。
- **M5StickV設定カード**：事後閲覧方式・プレビュー確認方式の2種類を切り替えます（詳細は第6章）。**この設定は写真の自動印刷・確認方式だけを切り替えるもので、俳句・ポエムの自動生成／自動印刷とは完全に独立しています**（俳句側は下の「俳句設定カード」で設定します）。加えて「回転」ボタンで写真の初期回転（0/90/180/270度、押すたびに時計回りへ90度ずつ進む）を設定できます。これは**次に受信するフレームから**適用される既定値で、カメラタブの「M5StickVカメラ」カードにある「回転」ボタン（今表示中の1枚だけをその場で回転させる操作）とは別物です。
...
- **外部データ転送カード（オプショナル）**：URLを入力すると、M5StickVで撮った写真・アップロードした写真を、撮影・選択のたびにそのURLへHTTP POST（JSON形式）で自動転送します。送信するJSONは`image`（プリント用に変換済みの白黒ビットマップをフルサイズのPNGにしたものをdata URLで）、`label`（M5StickVの検出ラベル、アップロード時はファイル名）、`location`（位置情報バッジで解決済みの地名、未解決なら空文字）、`capturedAt`（ISO 8601形式の日時）の4フィールドです。空欄なら何も転送されません。M5StickV設定カードの印刷モードや俳句設定カードの自動化設定とは完全に独立しており、印刷するかどうかに関わらず転送されます。URL自体はATOM Lite本体のNVSに保存されますが、転送処理そのものはこのページを開いているブラウザが行うため、m5webページを開いたままにしておく必要があります。
...
- **事後閲覧方式**：写真を受信次第すぐに印刷します。直近の1枚はカメラタブの「M5StickVカメラ」カードで確認できますが、印刷前の確認・キャンセルはできません。とにかく撮ったらすぐ紙が出てくる、という体験を重視したモードです。
...
写真があれば新しい順（m5webページのギャラリーと同じ並び）に1枚ずつ、画面全体を使って中央に大きく表示されます。位置・写真ID・検出ラベル（あれば）・保存日時は、写真に重ねる形で左上に小さく表示されます（白い帯を敷いた上に文字を乗せるので、写真の暗い部分に重なっても読めます）。以前のバージョンにあった画面下部のボタン案内（凡例）は廃止し、その分の高さも写真表示に充てています。

- **左ボタン（BtnA）＝前へ**：一覧を新しい方向へ1件戻ります（末尾では最古の1件から最新へ循環）。
- **中央ボタン（BtnB）＝次へ**：一覧を古い方向へ1件進みます（同様に循環）。
- **右ボタン（BtnC）＝短押しで印刷／長押しで俳句設定へ**：軽く押すと現在表示中の写真を再印刷します（`/api/gallery/print`と同じ動作）。約800ミリ秒以上押し続けると、後述の俳句設定画面に切り替わります。

どのボタンを押しても、押した合図として短いビープ音が鳴ります。BtnA/BtnBは700ミリ秒以内に同じボタンをもう一度押すと2回目は区別できる二重ビープが鳴り（結果は毎回1件ずつ進むだけなので、2回押せば自然と「次の次」「前の前」に到達します）、BtnCの短押しで印刷が成功すると画面に「Printed」と表示されるとともに、通常より大きな音の二重ビープが鳴ります。ビープ音が鳴っている間はRGB LEDも一瞬消灯し、鳴り終わると常時点灯の緑に戻ります。M5PaperColorはES8311コーデック＋1Wスピーカーを内蔵しているため、この音声フィードバックに追加の部品は不要です。

さらに、**BtnCを長押し（約800ミリ秒）する**と、m5webページの「俳句設定」カード（生成する詩の形式・著者名・印刷の自動化）をこの画面で確認できます。もう一度BtnCを長押しするとギャラリー表示に戻ります。

この画面が表示されている間は、ボタンの役割がギャラリー表示中とは変わります。

- **BtnCを短く押す**：俳句⇄ポエムを切り替えます。
- **BtnA／BtnB**：印刷の自動化（俳句を作らない／自動で俳句作成／自動でプリントまで実行）を前へ／次へと切り替えます。

著者名だけはこの画面から変更できません（タッチパネルもキーボードも無いため）——引き続きiPhone側のm5webページで設定してください。上記2項目はこの画面から直接`/api/haiku/settings`へPOSTされ、iPhone側の表示にもすぐ反映されます。
...
- **画面解像度と向き**：`SCREEN_W`/`SCREEN_H`は`setup()`内で実際の`M5.Display.width()/height()`から取得する方式に変更済みで、ハードコードした値ではありません（シリアルログの`[display] M5.Display reports WxH`で確認できます）。パネルの実際の向き・解像度に自動的に合わせるため、通常は調整不要です。
...
- **電源ボタン（`M5.BtnPWR`）は使用していません**：当初はこのボタンのダブルタップで俳句設定画面に切り替える設計でしたが、実機で確認したところ、押した瞬間にM5PM1電源ICのブラウンアウト検出が働きESP32がリセットされてしまい、ソフトウェア側にボタンイベントが一切届かないことが判明しました（シリアルログに`E BOD: Brownout detector was triggered`とだけ出て`wasPressed()`は一度も発火しない）。そのため、電源ボタンには一切触れず、BtnCの短押し／長押しで代替する設計に変更しています（前節参照）。
- **RGB LEDによる常時点灯・ビープ連動点滅**は本書執筆時点で実機検証中です。点灯しない場合はシリアルログの`[led] M5.Led.getCount()`の値を確認してください。0であればM5Unifiedがこの基板のLEDを自動認識できていないということなので、GPIO21に接続された2個のNeoPixelとして直接制御する方式（M5Stack公式のPaperColorデモファームウェアが採用している方式）に切り替える必要があります。
...
void loop() {
    ...
    handleButtons();   // BtnCの短押し／長押しで印刷・画面切替の両方を担う（7.3節）
    printHeartbeat();
...
3.4節・7.3節で触れたボタン割り当て（A＝前へ、B＝次へ、Cは短押しで印刷・長押しで俳句設定画面へ切替）の実装には、E Inkディスプレイの更新の遅さを見越した工夫が見られます。
...
**なぜ画面切り替えに電源ボタンを使わなかったのか**——当初はBtnA/Bと同じ「ダブルタップ」方式を本体上部の電源ボタン（`M5.BtnPWR`）に適用し、ギャラリー画面と俳句設定画面を切り替える設計でした。ところが実機でテストすると、電源ボタンを押した瞬間にシリアルログへ`E BOD: Brownout detector was triggered`とだけ出てボード自体がリセットされてしまい、`M5.BtnPWR.wasPressed()`は一度も発火しませんでした。M5PaperColorの電源ボタンは、ソフトウェアが読み取る前段で電源管理IC「M5PM1」の電源制御ロジックに直結しているため、押すこと自体が電源まわりの電圧降下（ブラウンアウト）を誘発してしまう、というハードウェア起因の制約です。そこで、すでに単押し・長押しの2種類を安定して検出できていたBtnC（`M5.BtnC.wasClicked()`／`wasHold()`、`setHoldThresh()`で長押しのしきい値を800msに設定）に画面切り替えの役割を移し、電源ボタンには一切触れない設計に変更しました。ライブラリのAPIドキュメント通りに実装しても、実機の電源回路設計まではドキュメントだけでは分からない——という典型的な例です。
...
カメラタブの「M5StickVカメラ」カード、アップロードタブの「画像を印刷」カード、どちらにも「俳句を作る」ボタンがあり、表示中の写真から季語入りの五七五俳句をその場で生成できます。裏側ではOpenAIの画像認識（vision）対応モデルを使っています。設定タブの「俳句設定」カードで「ポエム（30文字程度の自由詩）」を選んでいる場合は、ボタンの表示も「ポエムを作る」に変わり、生成される詩の行数・印刷時の列数もそれに合わせて変わります（形式の切り替えについては後述）。
...
- 設定タブ「デフォルト：明るさ・コントラストカード」の値が、M5StickVカメラと同じ扱いで
  サーバー側にそのまま適用されます（`src/jpeg_print.cpp`が`CameraLink::status()`から
  読み取ります）。写真アップロードのように個別調整はできないため、暗すぎる・薄すぎる場合は
  このカードの値自体を変更してください。
...
- **`esptool`が`Changing baud rate to 1500000...`の直後に`IndexError: index out of range`で落ちる場合**：`m5stack-atom`ボードの既定書き込み速度（1500000bps）が、USBシリアル変換チップ（CP210x）やケーブル・USBハブとの相性で不安定になっている可能性があります。`platformio.ini`の`[env:m5stack-atom]`に`upload_speed = 460800`（またはさらに遅い`115200`）を追加し、再試行してください。ボード自体やコードの問題ではなく、通信速度の相性による書き込み時のみのエラーです。
