...
// filename — arbitrary caption text doesn't fit the filesystem's filename
...
void begin() {
    prefs.begin("m5cam_gallery", false);
}
...
uint16_t beginSave(uint16_t height, const String &label) {
    ...
    writingFile = Storage::fs().open(path, "w");
    if (!writingFile) {
...
bool remove(uint16_t id) {
    ...
    bool ok = Storage::fs().remove(path);
    if (ok) {
